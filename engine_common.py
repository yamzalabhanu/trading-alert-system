...
# ===== OCC contract builder (inline to avoid importing polygon_client) =====
def _occ_strike_code(strike: float) -> str:
    # OCC strike portion: 8 digits, strike * 1000, zero-padded
    # 105.0 -> "00105000", 18.5 -> "00018500"
    try:
...
def build_option_contract(symbol: str, expiry_iso: str, side: str, strike: float) -> str:
    ...
    try:
        ...
        yy = d.strftime("%y"); mm = d.strftime("%m"); dd = d.strftime("%d")
    except Exception:
        # fallback to already-yyyymmdd-ish strings if needed
        s = re.sub(r"[^0-9]", "", str(expiry_iso))
        yy, mm, dd = s[2:4] or "00", s[4:6] or "00", s[6:8] or "00"
    right = "C" if str(side).upper() == "CALL" else "P"
    return f"O:{str(symbol).upper()}{yy}{mm}{dd}{right}{_occ_strike_code(strike)}"
...
# strike builder (local uses inline build_option_contract)
